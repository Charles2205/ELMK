...
- **`src/lib/actions.ts`**: `submitContactForm`, a single `'use server'` Server Action (zod-validated) shared by every `ContactForm` instance across the site (Collaborate, Mobility Programs, Sponsors pages) — instances are distinguished only by a hidden `context` form field, not separate endpoints. It currently just validates and `console.log`s the submission; wire up email/CRM delivery there when a provider is chosen.
